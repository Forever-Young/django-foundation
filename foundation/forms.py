...
def form_as_grid_pair_list(form):
    normal_row ='<div class="row formrow"><div class="small-4 small-offset-1 columns">'\
        + '%(label)s</div><div class="small-6 columns">%(field)s%(errors)s%(help_text)s'\
        + '</div><div class="small-1 columns">&emsp;</div></div>'
    output = form._html_output(
        normal_row = normal_row,
        error_row = u'<div class="hide">%s</div>',
        row_ender = '</div></div>',
